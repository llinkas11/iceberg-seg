...
SEARCH_WINDOWS = [
    # September — marginal SZA, keep a small baseline sample
    ("2019-09-01", "2019-09-30"),
    ("2021-09-01", "2021-09-30"),
    # October — SZA crosses 70°, core evaluation range
    ("2019-10-01", "2019-10-31"),
    ("2020-10-01", "2020-10-31"),
    ("2021-10-01", "2021-10-31"),
    ("2022-10-01", "2022-10-31"),
    # November — highest SZA (>75°)
...
MAX_RESULTS = 5        # per month per region — gives ~50 scenes total, ~25 GB
...
def main():
    ...
    print(f"\n  Downloading {len(scenes_to_download)} scene(s) into SZA bins...")
    for scene in scenes_to_download:
        ...
        try:
            # Refresh token before each download (tokens expire after ~10 min)
            token = get_access_token(COPERNICUS_USER, COPERNICUS_PASSWORD)
            download_product(scene["id"], scene["name"], token, out_dir)
        except Exception as e:
            print(f"  ✗ Failed to download {scene['name']}: {e}")
