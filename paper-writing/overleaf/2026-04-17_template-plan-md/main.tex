...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig-archive/20260430_082605__fig02_dataset_workflow.png}
\caption{\textbf{Fig. 2.} Dataset construction workflow. Two annotation sources are merged through three deterministic cleaning operations to produce the binary v4\_clean training, validation, and test splits. The shadow merge applies only to Fisser chips. The IC pixel mask is applied only to training chips with IC $\geq$ 15 \%; validation and test chips are never masked.}
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig-archive/20260430_082016__fig03_method_schematic.png}
\caption{\textbf{Fig. 3.} Six segmentation methods evaluated on the same test chip. Two methods operate directly on the input image (TR, OT). Four methods share the UNet++ probability map and differ only in post-processing (argmax, fixed threshold, per-chip Otsu, DenseCRF). One UNet++ training run produces all six method outputs.}
...
\textbf{Fixed B08 threshold (TR).} A scene-wide B08 $\geq$ 0.22 cutoff applied per chip without spatial context. This is the operational equivalent of the corrected-reflectance B08 $\geq$ 0.12 threshold reported by \citet{Fisser2024}. Polygons below the 100 m$^2$ minimum area are discarded after polygonisation.

\textbf{Per-chip Otsu (OT).} A chip-adaptive threshold computed from the B08 histogram \citep{Otsu1979}, with guards on radiometrically flat or ice-contaminated chips (Otsu floor 0.10, ceiling 0.50; chip skipped if IC $>$ 15 \%). The 0.10 floor is set in offset-uncorrected reflectance: a per-chip Otsu distribution computed over all 23,981 study chips with no floor applied shows a narrow spike just above 0.10 from the Sentinel-2 L1C baseline 04.00 +0.10 radiometric offset on ocean-only chips, which the floor removes (6.08 \%, 1,458 / 23,981 chips skipped); the offset-corrected equivalent of 0.10 (i.e.\ 0.20) would discard 50.54 \% of chips (12,120 / 23,981), including ~45 \% of chips with legitimate iceberg / water boundaries (Fig.~S1). Provides an image-only adaptive baseline against which to measure the value of the fixed Fisser threshold under SZA shifts.
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig-archive/20260430_085808__fig04_evaluation_schematic.png}
...
The paper is structured as a controlled progression rather than a flat method leaderboard. Phase A walks the dataset axis using lt65-scoped experiments: the original Fisser preprocessing, our 40 m and IC pipeline, null-chip injection, augmentations, and class / size balancing schemes. Each step changes one controlled variable, declared by a \texttt{controlled\_variable} field in the experiment YAML so the comparison cannot drift. Phase B walks the method axis on the canonical Phase A checkpoint, producing all six base method outputs from a single training run. Top-hat recovery is evaluated as a uniform post-inference extension to those base outputs.
...
\begin{figure*}[h]
\centering
\includegraphics[width=\textwidth]{figures/fig-archive/20260430_085902__fig05_progression.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig-archive/20260429_124124__mae_rootlen_vs_sza.png}
...
Table 2 reports per-pair mean Intersection over Union on matched pairs. UNet++ + Otsu leads in lt65; UNet++ + DenseCRF leads in 65--70$^\circ$; UNet++ alone leads in higher SZA bins. Fig.~\ref{fig:area_scatter} renders predicted vs reference iceberg area as a per-method scatter with the 1:1 line and a linear fit. Across the matched-pair pool, all methods correlate strongly with reference area (Pearson r in 0.94--0.99); the per-bin slope, intercept, and pair count are inset on each panel.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig-archive/20260429_225449__area_scatter_by_method.png}
\caption{\textbf{Fig. 7.} Per-pair predicted vs reference iceberg area, one panel per method, with the 1:1 line dashed and the linear fit colored to match the panel title. Points colored by SZA bin. Per-panel n, Pearson r, slope, and intercept are inset. Fisser (2024) Fig. 6 / Fisser (2025) Fig. 10 analog.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig-archive/20260429_225451__bias_delta_by_area.png}
\caption{\textbf{Fig. 8.} Top: per-pair bias (predicted minus reference iceberg area) by reference area bin and method, with the P10--P90 band shaded. Bottom: $\Delta$bias relative to the TR threshold baseline. Area bin edges (1.6k, 23.7k, 45.8k, 67.9k, 90k m$^2$) match Fisser (2025) Fig. 16. Per-bin total n\textsubscript{pairs} annotated.}
\label{fig:bias_delta_by_area}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig-archive/20260429_225524__re_by_area_bin.png}
\caption{\textbf{Fig. 9.} Per-pair relative error in iceberg area as a function of reference area bin, one line per method. Faint shaded band shows the P10--P90 spread per method per bin; mean is the solid line. Area bin edges match Fisser (2025) Fig. 12 / 16. Fisser (2024) Fig. 7 / Fisser (2025) Fig. 12 analog.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig-archive/20260430_093235__outline_examples.png}
\caption{\textbf{Fig. 10.} Per-SZA-bin chip examples with reference (cyan dashed) and UNet\_CRF predicted (magenta) iceberg outlines, picked at three per-pair RE positions: worst-positive, near-zero, worst-negative (chips with reference root length below 70 m excluded). B08 NIR backdrop, 256 $\times$ 256 px chips. Fisser (2024) Fig. 13 analog.}
...
The supplementary material contains one figure supporting the per-chip Otsu (OT) noise-floor decision in the Methods section.

\textbf{Fig.~S1.} Per-chip Otsu threshold distribution on Sentinel-2 B08 reflectance over all 23,981 study chips, with the production noise-floor of 0.10 (offset-uncorrected reflectance) and the alternative cutoffs 0.15 and 0.20 marked. Skip-rates: 0.10 removes 6.08 \% of chips (1,458 / 23,981), 0.15 removes 41.02 \% (9,838 / 23,981), and 0.20 (the offset-corrected equivalent of 0.10) removes 50.54 \% (12,120 / 23,981). The narrow spike just above 0.10 is the Sentinel-2 L1C baseline 04.00 +0.10 radiometric offset on ocean-only chips. The 0.10 cutoff in offset-uncorrected reflectance removes those noise-floor chips while preserving the 0.10 to 0.30 distribution mass that carries real iceberg / water boundaries; 0.20 would discard ~45 \% of chips with legitimate boundaries. Source script: \texttt{iceberg-rework/scripts/script\_check\_answers/q07\_otsu\_floor\_distribution.py}.
