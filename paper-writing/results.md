...
\paragraph{2026-05-05 supplementary follow-up.} The Phase A and Phase B re-evaluation across all four SZA bins on the v4\_clean test split (Slurm 60293 / 60296 / 60297) is intended for the supplementary material. Headline: among Phase A backbones, A1 (Fisser preprocessing $+$ 29 GT-zero chips) wins every higher-SZA bin on per-pair IoU and root-length MAE while A0 still wins lt65; aggregate over the three higher-SZA bins, A1 mean per-pair MAE 28.01 m vs A0 33.33 m. With Phase B re-run for both backbones on v4\_clean, A1 $+$ UNet\_CRF is the strongest single-backbone-single-method pipeline across all four SZA bins; A0 $+$ UNet\_OT remains the per-bin best for lt65 and reproduces the published 8.18 m headline within rounding (8.45 m on the v4\_clean lt65 split). The full T1--T4 tables (per-bin $\times$ per-experiment leaderboard, best non-A0 per bin, A0-vs-A1 method sweep, recommended pipeline) live in `shib\_end\_to\_end/phase\_a\_higher\_sza\_t1\_t4.md`. T1 and T2 are the supplementary figures that anchor this story; the main text retains the Fisser-comparable headline rather than the dataset progression. Top-hat (TH) variants were not included in the Phase B re-runs and are flagged as a remaining coverage gap.
