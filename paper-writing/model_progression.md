...
- **Phase B re-run with A7b complete (Slurm 60323, 7:20 wallclock; top-hat companions in 60328, 10:11 wallclock).** T4 cross-bin pipeline pick is now **A7b + UNet_CRF** (higher-SZA mean IoU 0.616 vs A1 + UNet_CRF's 0.602 at essentially tied MAE 15.59 m vs 15.21 m; A7b wins sza_gt75 outright on both metrics). Top-hat lifts UNet_OT recall in higher SZA but degrades UNet_CRF cross-bin, so A7b + UNet_CRF base (no TH) is the cleanest recommendation. lt65 still favours A0 + UNet_OT (8.45 m). Phase B + TH for the remaining 7 distinct backbones complete (Slurm 60337 + 60339); see `shib_end_to_end/phase_a_higher_sza_t1_t4.md` T3b for the all-10-backbone UNet_CRF comparison. A5a essentially ties A7b on 4-bin MAE (14.72 vs 14.75 m); A7b leads on 4-bin IoU (0.609 vs 0.606), so the recommendation stands.
