...
The five questions resolved here are removed from the reviewer pack so the external review focuses on the remaining unanswered questions; the historical question text and the empirical answers are preserved in `iceberg-rework/script-check-README.md` for traceability.
