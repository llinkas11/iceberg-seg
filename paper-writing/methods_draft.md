...
The 0.10 floor is set in offset-uncorrected reflectance, not in offset-corrected reflectance. An empirical check over all 23,981 study chips (per-chip Otsu computed on raw B08 with no floor applied; `iceberg-rework/scripts/script_check_answers/q07_otsu_floor_distribution.py`) shows that the per-chip Otsu distribution has a narrow spike just above 0.10 corresponding to the Sentinel-2 L1C baseline 04.00 +0.10 radiometric offset: ocean-only chips with no real iceberg signal return Otsu thresholds at the offset noise floor. The 0.10 cutoff removes 6.08 % of chips (1,458 / 23,981), almost all in this noise spike, while preserving the bulk of the distribution between 0.10 and 0.30, which contains real iceberg / water boundaries. Raising the floor to 0.20 (the offset-corrected equivalent of 0.10) would remove 50.54 % of chips (12,120 / 23,981), discarding ~45 % of chips with legitimate boundaries; an intermediate cutoff at 0.15 would remove 41.02 % (9,838 / 23,981). The 0.10 floor in offset-uncorrected reflectance is therefore the correct interpretation, and the 0.20 offset-corrected interpretation is rejected. See supplementary Fig. S1.
