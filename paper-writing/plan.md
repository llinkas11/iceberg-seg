...
**Status:** Refactor complete (Phases 1-6). v4_clean dataset built and frozen. baseline_v1 trained and evaluated. Phase A 2x3 grid (size balance x class balance) registered as 10 YAMLs, runner ready. Phase B method sweep is a single inference dispatch on the trained checkpoint, runner ready. Per-pair MAE + IoU tables published for the canonical baseline. **2026-05-05 addendum:** Phase A re-eval on all four SZA bins and Phase B backbone comparison (A0 vs A1) complete; A1 wins higher-SZA bins, A0 still wins lt65, A1 + UNet_CRF is the strongest single-pipeline option across all bins. **2026-05-05 evening update:** 8 A1-anchored Phase A variants trained and re-evaluated (A5a..A9a aug=off, A7b..A9b aug=on). A7b (== A8b == A9b by collapse) is the new higher-SZA champion: mean MAE 27.24 m vs A1's 28.01 m and A0's 33.33 m. A7b also beats A1 at lt65 (15.90 m vs 17.74 m); A0 still wins lt65 outright. See `shib_end_to_end/phase_a_higher_sza_t1_t4.md` for T1-T4.
